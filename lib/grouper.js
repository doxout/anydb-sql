...
function process(rows) {
    ...
    if (!checkKeys(rows[0], hasDot)) return rows;

    // Above 4 rows it pays off to build new functions

    if (rlen > 4)
        processed = removeDepth(fastMap(rows, makeNormalizer(rows[0])));
    else
        processed = removeDepth(fastMap(rows, normalize));
...
function createGroupedRowMaker(keys) {
    var fbody = "return {"
        + fastMap(keys.equal, createRegularField)
         .concat(fastMap(keys.other, createRegularField))
         .concat(fastMap(keys.group, createGroupField))
         .join(',')
    + '}';
